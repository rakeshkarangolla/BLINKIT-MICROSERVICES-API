...
import com.blinkit.cartservice.exception.InvalidTokenException;
 ...
 * <br>cart-service does not issue tokens; it only verifies them. The signing
 * key used for verification is loaded from the {@code JWT_SECRET} environment
 * variable, which is the SAME value auth-service uses to sign and
 * product-service uses to verify. This shared-secret model lets every
 * microservice independently authenticate a caller without contacting
 ...
 * forwarded verbatim by ProductServiceClient when cart-service calls
 * product-service on behalf of the user.
 *
